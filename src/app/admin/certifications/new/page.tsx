...
export default function NewCertificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const tagsArray = newCertification.tags
        .split(",")
        .map(tag => tag.trim())
      ...
      if (response.ok) {
        ...
        showNotification(`Failed to create certification: ${errorData.error || "Unknown error"}`, "error");
      }
    } catch (error) {
      showNotification("Error creating certification", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageSave = (imageUrl: string) => {
    setNewCertification({
      ...newCertification,
      imageUrl,
      imageAlt: newCertification.imageAlt || newCertification.title
    });
