...
export default function AdminCertificationsPage() {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error("Failed to fetch certifications:", errorData.error || "Unknown error", errorData.details || "");
          showNotification(`Failed to fetch certifications: ${errorData.error || "Unknown error"}`, "error");
        ...
        if (fetchError instanceof Error && fetchError.name === 'AbortError') {
          ...
          showNotification(`Error fetching certifications: ${errorMessage}`, "error");
  ...
  const confirmDeleteCertification = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/certifications?id=${certificationToDelete.id}`, {
        method: "DELETE",
      });

      if (response.ok) {
        setCertifications(certifications.filter((certification) => certification.id !== certificationToDelete.id));
        showNotification("Certification deleted successfully", "success");
      } else {
        showNotification("Failed to delete certification", "error");
      }
    } catch (error) {
      showNotification("Error deleting certification", "error");
    } finally {
  ...
  const handleImageSave = (imageUrl: string) => {
    ...
    // In a real implementation, you would update the certification with the new image URL
    ...
    <>
      ...
      {showImageEditor && (
        <CertificateImageEditor
          ...
          certificationId={certificationToDelete?.id}
