...
export default function CertificateImageEditor({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      xhr.open('POST', '/api/admin/certifications/upload-image');
      xhr.send(formData);
    } catch (error) {
      console.error("Error uploading image:", error);
      setError(error instanceof Error ? error.message : "An unknown error occurred");
    } finally {
      // Note: We don't set isUploading to false here because we're using XHR
      // The upload completion is handled in the XHR event listeners
    }
  };
            ...
            <p><span className="font-medium">Dimensions:</span>
              {previewUrl ? (
                <span className="ml-1">
                  <img
                    src={previewUrl}
                    alt="Hidden preview for dimensions"
                    className="hidden"
                    onLoad={(e) => {
                      ...
                      if (parent) {
                        ...
                        parent.innerHTML = `${width} × ${height}px ${
                          isAspectRatioRecommended
                            ? '<span class="text-green-600 dark:text-green-400">(✓ Recommended)</span>'
                            : '<span class="text-yellow-600 dark:text-yellow-400">(⚠ Not optimal)</span>'
                        }`;
                      }
                    }}
                  />
