...
export default function EditProjectPage() {
  ...
  const fetchProject = useCallback(async () => {
    ...
    if (Number.isNaN(numericId)) {
      showNotification("Invalid project identifier", "error");
    ...
    try {
      setLoading(true);
      const project = await getProjectByIdAction(numericId);
      if (!project) {
      ...
      showNotification(`Failed to fetch project: ${message}`, "error");
      router.push("/admin/projects");
    } finally {
      setLoading(false);
    }
  }, [projectId, router, showNotification]);
  ...
  useEffect(() => {
    ...
    void fetchProject();
  }, [projectId, fetchProject, router, showNotification]);
  ...
  const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    payload.append("projectId", numericId.toString());
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await updateProjectAction(numericId, payload);
  ...
  if (!projectId) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-600 dark:text-gray-300">Redirecting to projects...</p>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-[var(--color-gray-dark)]">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Update project information</p>
          </div>
          <div className="space-y-6">
            {/* Basic Information */}
