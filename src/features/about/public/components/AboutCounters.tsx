...
import { useInView, useMotionValue, useSpring } from "@/shared/ui/motion";
...
export default function AboutCounters({ data }: { data?: AboutCountersData }) {
  ...
  const satisfiedClients = Number.parseInt(data?.satisfiedClients ?? "", 10) || 0;
  const projectsCompleted = Number.parseInt(data?.projectsCompleted ?? "", 10) || 0;
  const yearsOfExperience = Number.parseInt(data?.yearsOfExperience ?? "", 10) || 0;
