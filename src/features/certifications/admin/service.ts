...
const CERTIFICATION_IMAGE_PREFIX = "certification";
...
export type UploadCertificationImageResult = {
  imageUrl: string;
  warning?: string;
};

export async function uploadCertificationImage(file: File): Promise<UploadCertificationImageResult> {
  ...
  const objectKey = generateUniqueFilename(file.name, CERTIFICATION_IMAGE_PREFIX);
...
export async function deleteCertificationImage(imageUrl: string): Promise<void> {
  const objectKey = extractObjectKeyFromUrl(imageUrl);
  if (!objectKey || !objectKey.startsWith(`${CERTIFICATION_IMAGE_PREFIX}-`)) {
    throw new Error("Not a certification image");
  }
